 ...
 */
class CFFClock {
	constructor(container, background = false, fps = false) {
		//Check if selected container is valid
		if (!document.getElementById(container)) {
			console.log("Select exactly 1 container to place the clock inside!");
			return false;
		}
		...
		c.wrapper.setAttribute("id", "cff_clock_wrapper");
		c.wrapper.setAttribute("style", "display:block; position: absolute; width: " + c.size + "px; height: " + c.size + "px; overflow: hidden; margin: 0;");
		c.face = c.wrapper.appendChild(c.createComponent("cff_clock_face", background ? c.svg.faceDark : c.svg.face));	//Build Face and Hands DOM Elements
		c.hoursHand = c.wrapper.appendChild(c.wrapper.appendChild(c.createComponent("cff_clock_hours", c.svg.hours)));
		c.minutesHand = c.wrapper.appendChild(c.createComponent("cff_clock_minutes", c.svg.minutes));
		c.secondsHand = c.wrapper.appendChild(c.createComponent("cff_clock_seconds", c.svg.seconds));
