...
function sbbUhr(container, background = false, fps = false) {
	...
	c.wrapper.setAttribute("style", "display:block; position: absolute; width: " + c.size + "px; height: " + c.size + "px; overflow: hidden; margin: 0; top: 50%; left: 50%; transform: translate(-50%, -50%);");
	
	// Add window resize listener to adapt size and maintain centered position
	window.addEventListener('resize', function() {
		c.size = Math.min(c.container.clientWidth, c.container.clientHeight);
		c.wrapper.style.width = c.size + 'px';
		c.wrapper.style.height = c.size + 'px';
	});
